...
function store($data){
	...
	<div class="store">
		...
		<img src="http://maps.google.com/maps/api/staticmap?center={GDIRECCION}&zoom=15&size=200x130&&markers=color:red|icon:http://www.asmred.com/img/marcaGoogleAsm01.png|size:mid|{GDIRECCION}&sensor=true&region=ES" />
	...
	$gdireccion = ($data['DIRECCIONWEB']) ? $data['DIRECCIONWEB'] : $data['DIRECCION'];
...
if(!$address) $address = (isset($_REQUEST['origin'])) ? $_REQUEST['origin'] : false;
...
if($postal){
	$postalXML = "http://172.20.20.15/WebSrvs/infoasm.asmx/Plazas?direccion=".$postal."&codpais=34";
...
if($province){
	$provinceXML = "http://172.20.20.15/WebSrvs/infoasm.asmx/Plazas?direccion=".$province."&codpais=34";
...
foreach($postalplazas as $plaza): 
		$args = array(
            ...
            'DIRECCIONWEB' => $plaza->DireccionWeb,
...
<?php } //if no address is provided show nothing ?>
